    ...
    if (!isUtilLoaded) {
        ...
        try {
            ...
            if (isCorsFetchError && isCloudflareAccessUrlInError) {
                ...
                // Call the UI prompt function via handleLoadingError
                if (window.AppUI && typeof window.AppUI.handleLoadingError === 'function') {
                    const cfError = new Error("Cloudflare Access potentially blocking critical static asset: " + localPath);
                    cfError.code = 'CLOUDFLARE_AUTH_REQUIRED';
                    cfError.data = { url: localPath };
                    AppUI.handleLoadingError(cfError, 'static_resource_load');
                } else {
                    // Fallback alert if AppUI or handleLoadingError is not available
                    alert("Your security session may have expired blocking essential resources. Please reload the page to re-authenticate.");
                }
...
window.AppActions = {
    ...
    _updateDeviceUI: function (data, lastUpdatedElement, listElement, noDevicesMessage, error = null) {
        ...
        try {
            ...
            if (error) {
                ...
                if (window.AppUI && typeof window.AppUI.handleLoadingError === 'function') {
                    // Determine context: if data is null (which it often is when _updateDeviceUI is called with an error),
                    // it might be an initial_load error. Otherwise, it's likely a refresh.
                    // fetchInitialData explicitly calls this with null data on error.
                    const context = (data === null && AppState.isInitialLoad) ? 'initial_load' : 'device_refresh';
                    AppUI.handleLoadingError(error, context);
                } else {
                    // Fallback if centralized handler isn't available (should not happen)
                    // Keep this minimal fallback for the very unlikely case AppUI.handleLoadingError is missing.
                    if (errorMessageElement) { errorMessageElement.textContent = `Error: ${error.message}`; errorMessageElement.style.display = 'block'; }
                    // The original AppUI.showErrorDialog call is removed from here as per requirement, 
                    // relying on the AppUI.handleLoadingError to show appropriate dialogs.
                }
                ...
                // Render empty list ONLY if the shared page is visible and an error occurred
    ...
    fetchInitialData: async function () {
        ...
        try {
            ...
            this._updateDeviceUI(null, lastUpdatedElement, listElement, noDevicesMessage, error);
            // The specific AppUI.showErrorDialog("Initial Load Failed", ...) call is removed.
            // AppUI.handleLoadingError called via _updateDeviceUI will handle the display.
        } finally {
            ...
            navigator.serviceWorker.addEventListener('message', (event) => {
                ...
                if (event.data && event.data.type === 'CLOUDFLARE_AUTH_REQUIRED') {
                    ...
                    if (!existingPrompt || !existingPrompt.classList.contains('show')) {
                        if (window.AppUI && typeof window.AppUI.handleLoadingError === 'function') {
                            const swCfError = new Error("Cloudflare re-authentication required by service worker for: " + (event.data.url || 'a resource'));
                            swCfError.code = 'CLOUDFLARE_AUTH_REQUIRED';
                            swCfError.data = { url: event.data.url };
                            AppUI.handleLoadingError(swCfError, 'sw_reported_cf_auth');
                        } else {
                            console.error("[App] AppUI or handleLoadingError not available when needed for SW Cloudflare error!");
                            // Fallback alert (less ideal)
                            alert("Your security session may have expired. Please reload the page to re-authenticate.");
                        }
                    } else {
                        console.log("[App] Cloudflare prompt already visible, ignoring duplicate message from SW.");
                    }
                }
                // --- SW Update Handling ---
